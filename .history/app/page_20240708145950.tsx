...
function Home() {
  ...
  const queryClient = useQueryClient(); // Use QueryClient from context
      ...
      <div className="bg-gradient-to-br from-black via-blue-950 to-black flex flex-1 flex-col p-12 lg:col-span-6">
        ...
        <div className="mt-10 flex flex-1 flex-col items-center space-y-6 text-center">
          <button className="py-2" onClick={handleButtonClick} id="mining-button">
            <h2 className="text-white text-3xl font-extrabold">Click Me</h2>
            <PencilSquareIcon className="h-12 w-12 mt-5 text-white" />
          </button>

          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col items-center">
              <ChartBarIcon className="h-12 w-12 text-blue-500" />
              <p className="text-white text-3xl font-bold">{totalPercentage.toFixed(2)}%</p>
              <p className="text-gray-500">Total Claimed</p>
            </div>
            <div className="flex flex-col items-center">
              <ChartBarIcon className="h-12 w-12 text-blue-500" />
              <p className="text-white text-3xl font-bold">{percentage.toFixed(2)}%</p>
              <p className="text-gray-500">Mining Progress</p>
            </div>
          </div>

          <div className="mt-10 text-center">
            <h3 className="text-white text-3xl font-extrabold">Mining Details</h3>
            <p className="text-gray-400 mt-2">Max Clicks: {maxClicks}</p>
            <p className="text-gray-400">Total Mined: {totalMined}</p>
            <p className="text-gray-400">Last Claim Time: {lastClaimTime ? new Date(lastClaimTime).toLocaleString() : 'N/A'}</p>
          </div>
        </div>
      </div>
    </main>
